...
export const VocabularyStage: React.FC<VocabularyStageProps> = ({ words, onComplete }) => {
  ...
  return (
    <div className="flex flex-col w-full max-w-4xl mx-auto min-h-[70vh]">
      {/* Navigation Bar */}
      <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 mb-4 sm:mb-10">
        {words.map((_, idx) => (
          <button
            key={idx}
            onClick={() => navigateTo(idx)}
            className={`w-8 h-8 sm:w-10 sm:h-10 aspect-square flex items-center justify-center rounded text-xs sm:text-sm font-bold cursor-pointer transition-all ${
              currentIndex === idx
      ...
      <div className="flex-grow bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="p-5 sm:p-8 md:p-12 flex-grow"
          >
            <div className="mb-6 md:mb-8 border-b border-slate-100 pb-6 md:pb-8 text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-3 md:gap-4 mb-3 md:mb-4">
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight text-slate-900 border-b-4 border-indigo-200 inline-block pb-1">{currentWord.word}</h2>
              ...
              {(currentWord.synonyms || currentWord.collocations) && (
                <div className="mt-5 md:mt-6 flex flex-wrap gap-4 md:gap-6 justify-center md:justify-start text-xs sm:text-sm">
                  {currentWord.synonyms && currentWord.synonyms.length > 0 && (
                    <div>
                      <span className="font-bold text-slate-400 uppercase tracking-widest text-[10px] sm:text-xs block mb-1">Synonyms</span>
                      <span className="text-slate-700">{currentWord.synonyms.join(', ')}</span>
                    </div>
                  )}
                  {currentWord.collocations && currentWord.collocations.length > 0 && (
                    <div>
                      <span className="font-bold text-slate-400 uppercase tracking-widest text-[10px] sm:text-xs block mb-1">Collocations</span>
                ...
                <div key={meaning.id} className="space-y-4">
                  <div className="flex gap-4">
                    <div className="flex-1 space-y-4">
                      <div>
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 sm:mb-3">Definition {mIndex + 1}</h3>
                        <p className="text-lg sm:text-xl text-slate-700 leading-relaxed">{meaning.definition}</p>
                        <TranslationToggle translations={meaning.translation} className="mt-2" />
                      </div>
                      
                      <div className="space-y-3 sm:space-y-4 pt-2 sm:pt-4">
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 sm:mb-3">Contextual Examples</h3>
                        {meaning.examples.map((example) => (
                          <div key={example.id} className="bg-slate-50 p-4 sm:p-5 rounded-xl border border-slate-100 group relative">
                            <div className="flex items-start justify-between gap-3 sm:gap-4 mb-2 sm:mb-3">
                              <TranslatableText targetWord={currentWord.word} text={example.sentence} className="text-slate-800 text-base sm:text-lg leading-snug flex-1" />
